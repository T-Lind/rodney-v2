# Significant credit to https://github.com/acheong08/ChatGPT for reverse engineering ChatGPT.
...
import rodney_ai.typings as tpng
...
class Chatbot:
    def __init__(
        ...
        if self.get_token_count("default") > self.max_tokens:
            raise tpng.ActionRefuseError("System prompt is too long")
    ...
    async def ask_stream_async(
        ...
        async with self.aclient.stream(
            ...
            async for line in response.aiter_lines():
                ...
                resp: dict = json.loads(line)
                if "error" in resp:
                    raise tpng.ResponseError(f"{resp['error']}")
                choices = resp.get("choices")
                if not choices:
                    continue
